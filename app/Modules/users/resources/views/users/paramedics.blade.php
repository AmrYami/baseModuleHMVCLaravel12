...
{{--            @if(auth()->user()->email == 'hr@fakeeh.com')--}}
                <button type="submit" class="btn btn-primary me-2"  name="DownloadMedia" value="1">
                    <i class="ki-duotone ki-magnifier fs-2"></i> @lang('common.Download Media')
                </button>
{{--            @endif--}}
            <hr>
            ...
            @foreach ($users as $user)
                @if($user->id != 1)
                    <tr>
                        ...
                        <td>{{ $user->mobile }}</td>
                        <td>{{  $user->company_id ? $user->company->name : 'System' }}</td>
                        <td>
                        {{
